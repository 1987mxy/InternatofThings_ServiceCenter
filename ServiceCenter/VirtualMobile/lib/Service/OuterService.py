...
class OuterService(Service):
	...
	def Key(self, data):
		...
		terminalInfoPackage = self.package.genPackage( self.mainThreadName, 'TerminalInfo', self.pid, terminalNames )
	...
	def QueryStatus(self, data):
		...
		terminalStatusPackage = self.package.genPackage( self.mainThreadName, 'TerminalStatus', self.pid, terminalStatus )
